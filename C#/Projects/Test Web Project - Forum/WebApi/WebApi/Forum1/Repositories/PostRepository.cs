...
namespace Forum1.Repositories
{
    public class PostRepository : IPostRepository
    {
        ...
        public List<Post> Get(PostQueryParameters queryParameters)
        {
            ...
            result = this.FilterByTitle(result, title);
        ...
        private static IQueryable<Post> SortBy(IQueryable<Post> result, string sortCriteria)
        {
            ...
            {
                case "id":
                    return result.OrderBy(post => post.Id);
                case "title":
                    return result.OrderBy(post => post.Title);
                case "username":
                    return result.OrderBy(post => post.UserId);
            }
            throw new ArgumentException("Sort critrtia not found");
        }

        private static IQueryable<Post> Order(IQueryable<Post> result, string sortOreder)
        {
            return (sortOreder == "desc") ? result.Reverse() : result;
        }
    }  
}