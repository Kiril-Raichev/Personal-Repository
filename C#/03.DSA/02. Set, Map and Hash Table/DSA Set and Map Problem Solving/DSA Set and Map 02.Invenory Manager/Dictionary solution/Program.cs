...
namespace TASK2InventoryManager
{
	internal class Program
	{
		...
		public static class ProductManager
		{
			...
			public static string AddProduct(string name, double price, string type)
			{
				...
				if (productsSet.Contains(newProduct))
				{
			...
			public static string FilterByType(string type)
			{
				...
				return "Ok: " + string.Join(", ", orderedItems.Select(x => $"{x.Name}({x.Price:F2})"));
			}
			...
			private static string OrderedResult(IEnumerable<Product> filteredItems)
			{
				...
				return "Ok: " + string.Join(", ", orderedResult.Select(x => $"{x.Name}({x.Price:F2})"));
			}
		}
	}
}